                ...
                if (cause.equals(DamageCause.BLOCK_EXPLOSION) || cause.equals(DamageCause.ENTITY_EXPLOSION)) {
                    if (event.getKiller() != null) {
                        if (event.getKiller().equals(event.getPlayer())) {
                            ...
                            deathMessage = name + ChatColor.RED + " is opgeblazen door " + Teams.getTeamColorByPlayer(event.getKiller()) + event.getKiller().getName() + ChatColor.GRAY + "'s TNT";
                        }
                    } else {
                    ...
                    if (event.getKiller() != null) {
                        if (event.getKiller().getItemInHand().getType().equals(Material.AIR)) {
                            ...
                            deathMessage = name + ChatColor.RED + " is vermoord door " + Teams.getTeamColorByPlayer(event.getKiller()) + event.getKiller().getName() + ChatColor.GRAY + "'s " + (event.getKiller().getItemInHand().getEnchantments() != null && event.getKiller().getItemInHand().getEnchantments().size() > 0 ? "enchanted " : "") + event.getKiller().getItemInHand().getType().name().replaceAll("_", " ").toLowerCase();
                        }
                    } else {
                        deathMessage = name + ChatColor.GRAY + " is verwond";
                    }
                    ...
                    if (event.getTrackerDamageEvent() != null) {
                        ...
                        deathMessage = name + ChatColor.GRAY + " viel van groote hoogte (" + fallDistance + " blocks)";
                    }
